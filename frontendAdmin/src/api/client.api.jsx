...
/**
 * Creates a new client application in the backend.
...
export const getAClients = async (queryData) => {
  try {
    ...
    const errorMessage =
      error.response?.data?.message || error.message || "Failed to retrieve client status.";
    throw new Error(errorMessage);
  }
};
...
export const updateClient = async (id, clientData) => {
  try {
    ...
    const errorMessage =
      error.response?.data?.message || error.message || "Failed to update client application.";
    throw new Error(errorMessage);
  }
};

/**
 * Authenticates admin credentials with backend .env credentials.
